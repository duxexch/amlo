    ...
    try {
      if (isLogin) {
        ...
        if (result.data.needsPinVerify) {
          // PIN login removed: always continue with email OTP second step.
          const otpResult = await loginOtpApi.sendOtp();
          setLoginOtpEmail(otpResult.email || "");
          setAuthSuccess(otpResult.devCode ? `${t("auth.otpTitle", "رمز التحقق")}: ${otpResult.devCode}` : null);
