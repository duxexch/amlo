...
function main() {
    ...
    const outPath = path.join(root, "qa", "results", "compatibility-summary.json");
    ...
    const fields = ["permissionsFlow", "backgroundBehavior", "batteryOptimization"] as const;
    const requiredHeaders = ["tier", ...fields];
    const headers = Object.keys(rows[0] || {});
    const missingHeaders = requiredHeaders.filter((h) => !headers.includes(h));
    if (missingHeaders.length > 0) {
        console.error("FAIL: device matrix missing Stage 47 columns");
    ...
    const permissionsExecuted = counters.permissionsFlow.pass + counters.permissionsFlow.fail;
    const backgroundExecuted = counters.backgroundBehavior.pass + counters.backgroundBehavior.fail;
    const batteryExecuted = counters.batteryOptimization.pass + counters.batteryOptimization.fail;
    ...
    const report = {
        ...
        compatibility: {
            permissionsFlow: {
                ...counters.permissionsFlow,
                executed: permissionsExecuted,
                successPct: pct(counters.permissionsFlow.pass, permissionsExecuted),
            },
            backgroundBehavior: {
                ...counters.backgroundBehavior,
                executed: backgroundExecuted,
                successPct: pct(counters.backgroundBehavior.pass, backgroundExecuted),
            },
            batteryOptimization: {
                ...counters.batteryOptimization,
                executed: batteryExecuted,
                successPct: pct(counters.batteryOptimization.pass, batteryExecuted),
            },
        },
        tier1: {
            checks: tier1Checks,
    ...
    console.log("=== AMLO Compatibility Audit (Stage 47) ===");
    console.log(`Matrix: ${matrixPath}`);
    console.log(`Report: ${outPath}`);
    console.log(`permissionsFlow pass/fail/pending: ${counters.permissionsFlow.pass}/${counters.permissionsFlow.fail}/${counters.permissionsFlow.pending}`);
    console.log(`backgroundBehavior pass/fail/pending: ${counters.backgroundBehavior.pass}/${counters.backgroundBehavior.fail}/${counters.backgroundBehavior.pending}`);
    console.log(`batteryOptimization pass/fail/pending: ${counters.batteryOptimization.pass}/${counters.batteryOptimization.fail}/${counters.batteryOptimization.pending}`);
